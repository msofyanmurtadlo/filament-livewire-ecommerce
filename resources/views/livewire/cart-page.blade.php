<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
    <div class="container mx-auto px-4">
        ...
        <div class="flex flex-col md:flex-row gap-4">
                        ...
                        <tbody>
                            @forelse ($cart_items as $item)
                                <tr wire:key='{{ $item['product_id'] }}'>
                                    ...
                                    <td class="py-4">
                                        {{ Str::of(Number::currency($item['unit_amount'], 'IDR', 'id'))->replace(',00', '') }}
                                    </td>
                                    <td class="py-4">
                                        <div class="flex items-center">
                                    ...
                                    <td class="py-4">
                                        {{ Str::of(Number::currency($item['total_amount'], 'IDR', 'id'))->replace(',00', '') }}
                                    </td>
                                    <td>
            ...
            <div class="md:w-1/4">
                <div class="bg-white rounded-lg shadow-md p-6" wire:ignore.self>
                    <h2 class="text-lg font-semibold mb-4">Summary</h2>
                    <div class="flex justify-between mb-2">
                        <span>Subtotal</span>
                        <span>{{ Str::of(Number::currency($grand_total, 'IDR', 'id'))->replace(',00', '') }}</span>
                    </div>
                    <div class="flex justify-between mb-2">
                        <span>Taxes</span>
                        <span>{{ Str::of(Number::currency(0, 'IDR', 'id'))->replace(',00', '') }}</span>
                    </div>
                    <div class="flex justify-between mb-2">
                        <span>Shipping</span>
                        <span>{{ Str::of(Number::currency(0, 'IDR', 'id'))->replace(',00', '') }}</span>
                    ...
                    <div class="flex justify-between mb-2">
                        ...
                        <span
                            class="font-semibold">{{ Str::of(Number::currency($grand_total, 'IDR', 'id'))->replace(',00', '') }}</span>
                    </div>
                    @if ($cart_items)
